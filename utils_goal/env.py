...
class Observation_WrapperMiniGrid(gym.core.Wrapper):
    ...
    def reset(self, **kwargs):
        ...
        if self.reached_goal>0:
            ...
            self.last_e_step=self.episode_step-self.step_goal_given
    ...
    def step(self, action):
        ...
        frame, reward, done, info = super().step(action.item())
        ...
        if compare_frame is not None:
            ...
            if diff_all is None:
                ...
                diff_type = len(diff_all) #take the diff location that did not appear in the previous location
        ...
        if self.goal is not None:
            goal=self.goal
            goal_frame=self.goal_frame.copy()
            goal_diff = self.goal_diff
            goal_step_given = self.step_goal_given
        ...
        return dict(
            ...
            diff=(int(diff)),
            diff_type=diff_type,
            goal=(goal if self.goal is not None else -1),
            reached_goal=reached_goal,
            reached_weight=(reached_weight if reached_goal else 0),
            seed=self.env_seed,
            goal_image=(goal_frame if self.goal_frame is not None else np.full_like(frame,-1)),
            goal_step=(goal_step_given if self.step_goal_given is not None else -1),
            goal_diff=(goal_diff if self.goal_diff is not None else -1),
            last_e_reached=0,
            last_e_step=-1,
            last_e_r_weight=0,
    ...
    def update_goal(self, goal_data):
        ...
        goal, goal_frame, goal_new, goal_diff= goal_data["goal"],goal_data["goal_frame"],goal_data["goal_new"],goal_data["goal_diff"]
        if goal_new:
            self.step_goal_given=self.episode_step
            self.reached_goal=0
            self.reached_weight = 0
            self.goal = goal
            self.goal_frame = goal_frame.copy()
...
def diff_frame(new_frame, frame, initial_frame, is_area_interes=False):
    ...
    ans=(np.sum(diff, -1) != 3).astype(np.float32) #Check one of the object,color or state has been changed in a location
    if ans.any():
